...
class Mama
{
    ...
    private function _crawlPregnantTips()
    {
        for ($i = 1; $i <= 40; $i++)
        {
            $url = str_replace('{which_week}', $i, $this->_crawl_urls['pregnant']);
            $this->_adapter_goutte->sendRequest($url);
            //main category
            $main_category_css_selector = 'div#headWeekNav.wrap.headWeekNav p em a.cur';
            $main_category = $this->_adapter_goutte->getText($main_category_css_selector);
            ...
            $sub_category = $this->_adapter_goutte->getText($sub_category_css_selector);//print_r($sub_category);exit;
            $sub_category_data = [
            ...
            $baby_title = $this->_adapter_goutte->getText($baby_title_css_selector);
            $baby_content = $this->_adapter_goutte->getHtml($baby_content_css_selector);
            $mom_title = $this->_adapter_goutte->getText($mom_title_css_selector);
            $mom_content = $this->_adapter_goutte->getHtml($mom_content_css_selector);
            $dad_title = $this->_adapter_goutte->getText($dad_title_css_selector);
            $dad_content = $this->_adapter_goutte->getHtml($dad_content_css_selector);
            $shopping_title = $this->_adapter_goutte->getText($shopping_title_css_selector);
            $shopping_content = $this->_adapter_goutte->getHtml($shopping_content_css_selector);
    ...
    private function _crawlBornFirstMonthTips()
    {
        for ($i = 1; $i <= 4; $i++)
        {
            $url = str_replace('{which_week}', $i, $this->_crawl_urls['born_first_month']);
            $this->_adapter_goutte->sendRequest($url);
            //main category
            $main_category_css_selector = 'div#headWeekNav.wrap.headWeekNav p em a.cur';
            $main_category = $this->_adapter_goutte->getText($main_category_css_selector);
            ...
            //sub category
            $sub_category_css_selector = 'div.headWeekBox h1';
            $sub_category = $this->_adapter_goutte->getText($sub_category_css_selector);
            $sub_category_data = [
                'msc_name' => $sub_category[0],
                'msc_main_category' => $main_category[0],
                'msc_url' => $url
            ...
            $ability_title = $this->_adapter_goutte->getText($ability_title_css_selector);
            $ability_content = $this->_adapter_goutte->getHtml($ability_content_css_selector);
            $health_title = $this->_adapter_goutte->getText($health_title_css_selector);
            $health_content = $this->_adapter_goutte->getHtml($health_content_css_selector);
            $nutrient_title = $this->_adapter_goutte->getText($nutrient_title_css_selector);
            $nutrient_content = $this->_adapter_goutte->getHtml($nutrient_content_css_selector);
            $shopping_title = $this->_adapter_goutte->getText($shopping_title_css_selector);
            $shopping_content = $this->_adapter_goutte->getHtml($shopping_content_css_selector);
            $game_title = $this->_adapter_goutte->getText($game_title_css_selector);
            $game_content = $this->_adapter_goutte->getHtml($game_content_css_selector);
    ...
    private function _crawlBornRemainMonthsTips()
    {
        for ($i = 2; $i <= 12; $i++)
        {
            $url = str_replace('{which_month}', $i, $this->_crawl_urls['born_remain_month']);
            $this->_adapter_goutte->sendRequest($url);
            //main category
            $main_category_css_selector = 'div#headWeekNav.wrap.headWeekNav p em a.cur';
            $main_category = $this->_adapter_goutte->getText($main_category_css_selector);
    ...
    private function _crawlBornYearMonthsTips()
    {
        for ($i = 1; $i <= 2; $i++)
        {
            ...
            for ($j = 1; $j <= 12; $j++)
            {
                $url = str_replace(array('{which_year}', '{which_month}'), array($i, $j), $this->_crawl_urls['born_year_month']);
                $this->_adapter_goutte->sendRequest($url);
                //main category
                ...
                $this->_adapter_db->insert($this->_table_names['sub_category'], $sub_category_data);
                //article
                $titles = $this->_adapter_goutte->getHtml($title_css_selector);
